...
[[_debris_branches,_rain_fog],"AL_scripts\monsoon\alias_monsoon_effect.sqf"] remoteExec ["execVM",0,true];
if(_rain_drop) then {[[],"AL_scripts\monsoon\rain_drop.sqf"] remoteExec ["execVM",0,true]};
if(_thunder_steroids) then {
		_rand_pl = [] execVM "AL_scripts\monsoon\alias_hunt.sqf";
...
[] spawn {
	...
	while {al_monsoon_om} do 
	{
		...
		[[poz_f_1,_tip_tunet,_numar_sclipiri,_sound_tunet,_storm_clouds],"AL_scripts\monsoon\thunder_storm.sqf"] remoteExec ["execVM"];
		sleep _delayth2;
	};
};
};
...
if (_effect_on_objects) then {
	...
	// interval object blow
		...
		[] spawn {
			...
			["AL_scripts\monsoon\camera_shake.sqf"] remoteExec ["execVM"];
		};
